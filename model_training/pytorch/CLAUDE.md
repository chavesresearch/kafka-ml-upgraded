# model_training/pytorch — status and continuation notes
...
Sibling of `../../../kafka-ml/model_training/pytorch` (kept untouched as reference/
rollback), same pattern as `tensorflow/` in this same directory,
`backend`, `mlcode_executor/pthexecutor`: a behaviorally
...
- `avro.schema`/`avro.io.DatumReader`/`BinaryDecoder` - **still works
  ...
  lesson already learned in `mlcode_executor/pthexecutor`'s port):
...
- **Important**: `TensorboardLogger`/`ModelCheckpoint`/the `ignite.metrics.*`
  ...
  `mlcode_executor/CLAUDE.md` for `tfexecutor`/`pthexecutor`'s
...
- `pyproject.toml`/`uv.lock` (new uv project, `requirements.txt` deleted).
  ...
  `requests==2.32.5` - all copied from `mlcode_executor/
...
(which round-trips through `mlcode_executor/tfexecutor` to
...
above)/`Deployment`/`TrainingResult` chain directly in `backend`'s
...
loop** (confirms the earlier `backend` `status_code=200` fix on
...
- **AVRO input format** (`TrainingKafkaDataset.__decodedata__`'s
  ...
  2. `DatumReader(data_scheme)` is called with a raw JSON-ish string, not
     ...
     `mlcode_executor/CLAUDE.md` for `tfexecutor`'s `JsonDecoder`.
  ...
  byte-for-byte unchanged from `../../../kafka-ml/model_training/pytorch/
