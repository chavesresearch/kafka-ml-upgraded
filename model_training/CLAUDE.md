# model_training — instructions for AI assistants

A behaviorally faithful port of the original `model_training` (preserved
at `../../kafka-ml/model_training` as historical reference), same pattern
as `backend`/`mlcode_executor`/`datasources`: modern dependency versions,
fixing only what's actually broken by the upgrade - not a rewrite or a
redesign. `training.py`-equivalent files in each subdirectory dispatch on
the `CASE` env var (1-9) to one of 9 training modes: single/distributed ×
classic/incremental, 4 federated variants, and 1 blockchain-federated
variant. **All 9 must keep working** - this was an explicit,
non-negotiable requirement from the user for the TensorFlow pass, and
applies equally to PyTorch.
...
4. **Real end-to-end testing catches things import-checks can't.** The
   TensorFlow pass found a live cross-service bug this way (a `backend`
   HTTP status-code mismatch causing an infinite client-side retry loop -
   fixed in `backend`, documented in `tensorflow/CLAUDE.md`) that no
   amount of reading the training container's own code in isolation would
   have surfaced.
