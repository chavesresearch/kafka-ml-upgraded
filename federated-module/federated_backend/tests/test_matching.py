"""Unit tests for the pure matching logic in app/matching.py -
check_colission() and is_blank() - no DB or Kafka needed. Ported unchanged
from the original Django test_check_colission.py.

check_colission() compares `dataset_restrictions`/`data_restriction` via
json.loads() on both sides. federated-module/CLAUDE.md documents a real
incident where this looked like a bug (a first-pass smoke test posted a
native dict and json.loads() crashed on it) and was almost "fixed" by
deleting the json.loads() calls - wrong, because the real clients
(FederatedRawSink, the main trainer's DATA_RESTRICTION env var) always
send this field as a JSON-encoded *string*. These tests lock in that real
(string-encoded) contract so this doesn't get "fixed" again by mistake.
"""

from app.matching import check_colission, is_blank
