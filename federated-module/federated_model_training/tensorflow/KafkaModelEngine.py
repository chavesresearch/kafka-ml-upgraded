...
class KafkaModelEngine():
      ...
      def __getModelWeights__(self, controlMessage):
        ...
        for kafkaControl in self.__splitPartitionsIntoControlMsgs__(controlMessage):
            ...
            # aggregation round) - unsubscribe() alone leaves the underlying
            # socket/threads open, leaking one of each per round over a long
            # federated run. close() releases them; wrapped in finally so an
            # exception mid-read still cleans up.
            try:
