...
class KafkaMLSink(object):
    ...
    Args:
        ...
        control_topic (str): Control Kafka topic for sending confirmation after sending training data.
            Defaults to control
        group_id (str): Group ID of the Kafka consumer used to look up partition offsets. Defaults to sink
    ...
    Attributes:
        ...
        __consumer (:obj:KafkaConsumer): Kafka consumer, used only to look up partition offsets
    ...
    @staticmethod
    def __deployment_id_to_bytes(deployment_id):
        """Encodes the deployment id as a 4-byte big-endian key for the control
        topic message.

        Widened from the original single-byte `bytes([deployment_id])`
        encoding, which raised `ValueError` for any deployment id >= 256.
        Every consumer of this key (`kafka_control_logger`,
        `mlcode_executor/tfexecutor`, `model_training/*`) decodes it
        generically via `int.from_bytes(msg.key, byteorder="big")`, so
        widening the width here is wire-compatible regardless of which of
        them reads it - none of them assume a fixed byte length.

        Deliberately separate from `__object_to_bytes`: that method encodes
        *labels* for data messages (a different wire contract entirely,
        decoded by `mlcode_executor`/training containers as single-byte
        int/bool, 4-byte float, or utf-8 str) and must not change.
        """
        return int(deployment_id).to_bytes(4, byteorder='big')

    def __get_partitions_and_offsets(self, topic):
        """Obtains the current end offset of every partition in the topic defined.

        Uses `KafkaConsumer.end_offsets()` to fetch every partition's offset
        in a single batched call, rather than the `assign()` +
        `seek_to_end()` + `position()` sequence repeated once per partition
        (each of those being its own broker round trip, and `assign()`
        mutating the shared consumer's partition assignment on every call
        for no reason - this consumer is never used to actually read
        messages).
        """
        ...
        if partitions is not None:
            ...
            end_offsets = self.__consumer.end_offsets(topic_partitions)
            for tp, offset in end_offsets.items():
                dic[tp.partition] = {'offset': offset}
        return dic
    ...
    def __send_control_msg(self):
        ...
        key = self.__deployment_id_to_bytes(self.deployment_id)
