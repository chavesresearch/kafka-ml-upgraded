...
- `decoders.py`'s `AvroDecoder` switched from `tensorflow-io`'s Avro op to `fastavro` (`tensorflow-io` hasn't shipped a release since mid-2023 and caps out at TF 2.16) - same replacement already applied in `model_training-upgraded/tensorflow` and `mlcode_executor-litestar/tfexecutor`.
