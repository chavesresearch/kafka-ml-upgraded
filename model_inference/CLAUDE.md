# model_inference — instructions for AI assistants

A faithful port of the original `model_inference`, preserved at
`../../kafka-ml/model_inference` as historical reference. Same
faithful-port philosophy as `model_training`: fix only what each
dependency upgrade actually broke, verify empirically. Much smaller scope
than `model_training` - no `CASE` dispatch, no per-mode class hierarchy,
just one `inference.py` per framework that does real-time single-message
Kafka consume -> `model.predict()`/`model(x)` -> Kafka produce, in a plain
`while True: poll()` loop via `confluent_kafka`.

**Status: this is the deployed inference service.** Both `tensorflow/` and
`pytorch/` are functionally complete and verified end-to-end against the
live local cluster (using the already-trained models from
`model_training`'s own verification pass - `result_id=1` for TensorFlow,
`result_id=7` for PyTorch).
...
- `requirements.txt` -> `pyproject.toml`/`uv.lock`. Pins:
  ...
  `model_training/tensorflow` - `tensorflow/tensorflow:2.21.0`'s
  ...
  `model_training/tensorflow/pyproject.toml` reflexively, check
...
- **No Keras 3 `load_model()`/optimizer fix needed here** (unlike the
  ...
  `model_training/tensorflow/CLAUDE.md`) doesn't apply; verified
...
- Dockerfile bumped `tensorflow/tensorflow:2.7.0` -> `2.21.0`, uv-based
  layer pattern matching `model_training/tensorflow`.

## pytorch/

- `requirements.txt` -> `pyproject.toml`/`uv.lock`. Pins copied from
  `model_training/pytorch`'s already-settled, already-verified
...
- `avro`/`pytorch-ignite` - confirmed compatible unchanged (same finding as
  `model_training/pytorch`'s CLAUDE.md - no code changes needed,
...
- **AVRO input format is dead code here too, same as the training side**:
  ...
  message, pre-existing (byte-identical to `../../kafka-ml/model_inference/pytorch/
  decoders.py`), not fixed, matches the same "flag don't fix" stance
  applied to the identical bug shape in `model_training/pytorch`.
...
results from `model_training`'s own test pass (TensorFlow
...
- **PyTorch**: `{"values": [[[-0.343..., -0.204...]]]}` - correct raw
  ...
  Byte-identical to `../../kafka-ml/model_inference/pytorch/inference.py` - a
...
- **Distributed inference** (the `distributed=True` branch in
  ...
  training side (`model_training/tensorflow/CLAUDE.md`'s CASE=3
